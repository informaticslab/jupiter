...
d3.json("/apollo/api/node/viewer/" + id, function(error, json) {
	...
	function hidenodes(nodetype) {
		nodetype = "Collaborative";
		console.log(nodetype);
		d3.selectAll("circle.node." + nodetype).style({
			'visibility': 'hidden'
		});
		d3.selectAll("text.nodetext." + nodetype).style({
			'visibility': 'hidden'
		});
		d3.selectAll("path.link." + nodetype).style({
			'visibility': 'hidden'
		});
		d3.selectAll("text.path_label." + nodetype).style({
			'visibility': 'hidden'
		});
